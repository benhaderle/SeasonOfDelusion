...
namespace Shapes {
	...
	public static class ShapesMeshGen {
		...
		public static Mesh GenLineMapMesh(Mesh mesh, IList<MapPoint> mapPoints, PolylineJoins joins, bool flattenZ, bool useColors)
		{
			meshColors.Clear();
			meshVertices.Clear();
			meshUv0.Clear();
			...
			int pointCount = mapPoints.Count;

			if (pointCount < 2)
				return mesh;

			MapPoint firstPoint = mapPoints[0];
			MapPoint lastPoint = mapPoints[mapPoints.Count - 1];

			// if the last point is at the same place as the first and it's closed, ignore the last point
			if (pointCount == 2 && SamePosition(firstPoint.point, lastPoint.point))
			{
				pointCount--; // ignore last point
				if (pointCount < 2) // check point count again
					return mesh;
				lastPoint = mapPoints[mapPoints.Count - 2]; // second last point technically
			}
			...
			for (int i = 0; i < pointCount; i++)
			{
				...
				float pathThickness = mapPoints[i].thickness;

				// Indices & verts
				Vector3 vert = flattenZ ? new Vector3(mapPoints[i].point.x, mapPoints[i].point.y, 0f) : mapPoints[i].point;
				Color color = useColors ? mapPoints[i].color.ColorSpaceAdjusted() : default;
				...
				if (i == 0)
				{
					prevPos = firstPoint.point * 2 - mapPoints[1].point; // Mirror second point
					nextPos = mapPoints[i + 1].point;
				}
				else if (i == pointCount - 1)
				{
					prevPos = mapPoints[i - 1].point;
					nextPos = mapPoints[pointCount - 1].point * 2 - mapPoints[pointCount - 2].point; // Mirror second last point
				}
				else
				{
					prevPos = mapPoints[i - 1].point;
